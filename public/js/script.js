...
document.addEventListener('DOMContentLoaded', function () {
  ...
  btn.addEventListener('click', function () {
    ...
    const price = parseFloat(row.querySelector('td:nth-child(3)').textContent.replace('₹', '').replace(/,/g, ''));
    ...
    .then(data => {
      if (data.status === 'success') {
        ...
        // Update total (recalculate)
        let total = 0;
        document.querySelectorAll('tbody tr').forEach(tr => {
          const sub = tr.querySelector('td:nth-child(5)').textContent.replace('₹', '').replace(/,/g, '');
