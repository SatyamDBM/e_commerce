    ...
    <div class="col-lg-7 col-md-6">
      ...
      <div class="mt-5 border-top border-secondary pt-4">
        ...
        <p class="text-secondary mb-0">
          Usually delivered within <strong>3–5 business days</strong> across India.
        </p>
  ...
  <div class="row g-3 justify-content-center">
    @foreach(\App\Models\Product::where('id', '!=', $product->id)->take(4)->get() as $related)
      <div class="col-lg-3 col-md-4 col-sm-6 col-6">
        <div 
          class="card bg-dark border-0 shadow-sm product-card h-100 small-card"
          data-product-id="{{ $related->id }}">
          
          <div class="ratio ratio-1x1 overflow-hidden">
            <img 
              src="{{ asset('uploads/' . $related->product_image) }}"
              class="card-img-top object-fit-cover"
              alt="{{ $related->name }}">
          </div>

          <div class="card-body text-center py-3">
            <h6 class="fw-semibold text-light text-truncate">
              {{ $related->name }}
            </h6>
            <p class="text-success fw-semibold mb-2 small">
              ₹{{ number_format($related->offer_price) }}
            </p>
            <a 
